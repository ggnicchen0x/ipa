...
enum DevicePatchService {
    static func apply(project: PatchProject) throws -> PatchTransactionReceipt {
        ...
        guard !roots.isEmpty else {
            throw PatchPackageError.targetAppUnavailable(bundleIDs.first ?? "")
        }

        var targetProject = project
        targetProject.bundleIdentifiers = project.bundleIdentifiers.filter { roots[$0] != nil }
        targetProject.directories = project.directories.filter { roots[$0.bundleID] != nil }
        targetProject.rules = project.rules.filter { roots[$0.bundleID] != nil }
    ...
    static func inspectRestore(receipt: PatchTransactionReceipt) throws -> PatchRestoreInspection {
        ...
        return try PatchTransaction.inspectRestore(
            receipt: receipt,
            containerResolver: { bundleID in
                guard let root = roots[bundleID] else {
                    throw PatchPackageError.targetAppUnavailable(bundleID)
